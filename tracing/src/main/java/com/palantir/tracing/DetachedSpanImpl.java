...
final class DetachedSpanImpl implements DetachedSpan {
    ...
    public CloseableSpan attach() {
        if (span.metadata().isPresent()) {
            span.attach();
            return span::detach;
        } else {
            // TODO(pkoenig): This sucks, should we just do this unconditionally?
            Tracer.pushEntry(traceState);
            return () -> Tracer.removeEntry(traceState);
        }
    }
